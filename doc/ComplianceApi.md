...
final api = MudbaseSdk().getComplianceApi();
final ApiGdprErasePostRequest apiGdprErasePostRequest = ; // ApiGdprErasePostRequest | 

try {
    final response = api.apiGdprErasePost(apiGdprErasePostRequest);
    print(response);
} on DioException catch (e) {
    print('Exception when calling ComplianceApi->apiGdprErasePost: $e\n');
}
```
...
try {
    final response = api.apiGdprExportGet();
    print(response);
} on DioException catch (e) {
    print('Exception when calling ComplianceApi->apiGdprExportGet: $e\n');
}
```
...
final api = MudbaseSdk().getComplianceApi();
final GenerateAccessReviewRequest generateAccessReviewRequest = {"orgId":"685acbe0e129932fbb7a0fc3","reviewPeriod":{"start":"2024-10-01T00:00:00.000Z","end":"2024-12-31T23:59:59.000Z"}}; // GenerateAccessReviewRequest | 

try {
    final response = api.generateAccessReview(generateAccessReviewRequest);
    print(response);
} on DioException catch (e) {
    print('Exception when calling ComplianceApi->generateAccessReview: $e\n');
}
```
...
final api = MudbaseSdk().getComplianceApi();
final GenerateDataProcessingRecordRequest generateDataProcessingRecordRequest = {"orgId":"685acbe0e129932fbb7a0fc3","recordDate":"2024-12-16T00:00:00.000Z"}; // GenerateDataProcessingRecordRequest | 

try {
    final response = api.generateDataProcessingRecord(generateDataProcessingRecordRequest);
    print(response);
} on DioException catch (e) {
    print('Exception when calling ComplianceApi->generateDataProcessingRecord: $e\n');
}
```
...
try {
    final response = api.getComplianceSummary();
    print(response);
} on DioException catch (e) {
    print('Exception when calling ComplianceApi->getComplianceSummary: $e\n');
}
```
...
final api = MudbaseSdk().getComplianceApi();
final LogSecurityEventRequest logSecurityEventRequest = {"eventType":"unauthorized_access_attempt","severity":"high","details":{"userId":"685acbe0e129932fbb7a0fc2","resource":"admin-panel","ipAddress":"192.168.1.100","action":"blocked","reason":"Insufficient permissions"}}; // LogSecurityEventRequest | 

try {
    final response = api.logSecurityEvent(logSecurityEventRequest);
    print(response);
} on DioException catch (e) {
    print('Exception when calling ComplianceApi->logSecurityEvent: $e\n');
}
```
